...
type ContactUniforms = {
	...
	uSoftness: THREE.Uniform<number>;
	uRadius: THREE.Uniform<number>;
	uAspect: THREE.Uniform<number>;
};
...
const Scene: React.FC<SceneProps> = ({ moods, targetAngle }) => {
	...
	useFrame(({ clock, size }, delta) => {
		...
		const targetPalette = paletteAtAngle(
			angleNow,
			moods,
			moodColorPalettes
		);

		(uniforms.color1.value as THREE.Color).lerp(
			targetPalette.color1,
			alpha
		);
		(uniforms.color2.value as THREE.Color).lerp(
			targetPalette.color2,
			alpha
		);
		(uniforms.color3.value as THREE.Color).lerp(
			targetPalette.color3,
			alpha
		);
		(uniforms.color4.value as THREE.Color).lerp(
			targetPalette.color4,
			alpha
		);

		const deltaDeg = shortestAngleDelta(visualAngle.current, angleNow);
		visualAngle.current =
			Math.abs(deltaDeg) > 0.001
				? (visualAngle.current + deltaDeg * alpha + 360) % 360
				: angleNow;
		...
		<>
			...
			>
				...
				<shaderMaterial
					...
					uniforms={contactU as ContactUniforms}
					transparent
					depthWrite={false}
				/>
			</mesh>
	...
	return (
		<div className="flex flex-col items-center">
			<h2 className="text-[24px] font-extrabold text-dark-text mb-4">
				Today I&apos;m feeling{" "}
				<span className="lowercase">{selectedMoodLabel}</span>
			</h2>
			<div
				ref={containerRef}
