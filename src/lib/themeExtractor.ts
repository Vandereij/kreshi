...
export async function extractThemes(
  ...
  const stopWords = new Set<string>([
    ...
    "its","he","she","him","her","we","us","you","your","they","them",
  ...
  // --- Step 1: Compromise entities & noun phrases ---
  const compromiseEntities: string[] = [];
  doc.people().out('array').forEach((p: string) => compromiseEntities.push(p));
  ...
  compromiseEntities.forEach((entity: string) => {
    ...
    if (cleanEntity && cleanEntity.length > 2 && !stopWords.has(cleanEntity)) {
      themeCounts.set(cleanEntity, (themeCounts.get(cleanEntity) || 0) + 3);
    }
  });
  ...
  recentEntries.forEach((entry, idx) => {
    const splits = (entry.text || '').split(/(?<=[.!?])\s+/g).filter(Boolean);
    splits.forEach((s: string) => sentences.push({ text: s, date: new Date(entry.date), entryIndex: idx }));
  });
  ...
  recentEntries.forEach((entry, idx) => {
    ...
    const w = Math.exp(-Math.max(0, (now.getTime() - entryDate.getTime())) / (tauDays * dayMs));

    for (const t of themes) {
      if (entryText.includes(t)) {
        const s = stats.get(t) ?? { theme: t, freq: 0, coverage: 0, recencyWeighted: 0, entries: new Set<number>() };
        s.freq += 1;
        s.recencyWeighted += w;
        s.entries.add(idx);
        stats.set(t, s);
  ...
  for (const [t, s] of stats) {
    if (s.freq < 2 || t.length < 3) stats.delete(t);
  }
  ...
  for (const [t, s] of stats) {
    ...
    score.set(t, 0.35 * freqN + 0.35 * covN + 0.30 * recN);
  }

  // === MMR diversity ===
  const lambda = mmrLambda;
  const pool = Array.from(stats.keys()).sort((a, b) => (score.get(b)! - score.get(a)!));
  ...
  if (useEmbeddings) {
    try {
      ...
      const sentMatrix: number[][] = (await embedder(
        limitedSentences.map((s) => s.text),
        { pooling: 'mean', normalize: true }
      )).data;
      const sentenceVectors: Float32Array[] = sentMatrix.map((row) => Float32Array.from(row));

      candVec = new Map<string, Float32Array>();
      for (const t of pool) {
        const idxs: number[] = [];
        for (let i = 0; i < limitedSentences.length; i++) {
          if (limitedSentences[i].text.toLowerCase().includes(t)) idxs.push(i);
        }
        if (idxs.length === 0) continue;
        candVec.set(t, normalize(avgVectors(idxs.map((i) => sentenceVectors[i]))));
      }
    } catch {
    ...
    for (const t of pool) {
      ...
      if (selected.length > 0) {
        if (candVec) {
          ...
          redundancy = Math.max(...selected.map((s) => jaccard(sT, stats.get(s)!.entries)));
        }
      }
